		...
		switch (lexems.lexems_[state].kind) {
			case VAR:
				...
				if (lexems.lexems_[state].kind == OP && lexems.lexems_[state].lex.op == EQUAL)
					...
					std::cout << "eval - unexpected lexem" << ": expected EQUAL operation"<< std::endl;
					exit(-1);
				}
				if (lexems.lexems_[state].kind == T || lexems.lexems_[state].kind == F)
					...
					std::cout << "eval - unexpected lexem" << ": expected true or false value"<< std::endl;
					exit(-1);
				}
				++state;
				break;
			default:
				...
				std::cout << "eval lexer error - unexpected lexem" << ": expected VariableName"<< std::endl;
				exit(-1);
		}
	}
...
node_t *eval_t::parse_eval (node_t *current) {
	...
	if (temp1->data.k != NODE_VAL || temp2->data.k != NODE_VAL) {
		...
		if (current->data.u.op == AND && (temp1->data.value == 1 || temp2->data.value == 1)) {
			if (temp1->data.value == 1)  { delete temp1; delete current; return temp2; }
			else                         { delete temp2; delete current; return temp1; }
		}
		...
		if (current->data.u.op == OR && (temp1->data.value == 0 || temp2->data.value == 0)) {
			if (temp1->data.value == 0) { delete temp1; delete current; return temp2; }
			else                        { delete temp2; delete current; return temp1; }
		}
		
		if (current->data.u.op == IMPL && temp1->data.value == 1) {
			delete temp1; delete current; return temp2;
		}
	...
	switch (current->data.u.op) {
		case AND: new_node->data.u.val = ((temp1->data.u.val) & (temp2->data.u.val)); break;

		case OR : new_node->data.u.val = ((temp1->data.u.val) | (temp2->data.u.val)); break;
